...
for cell in cells(mesh): # set the characteristic functions
  if cf[cell] == 0:
    print chi0.vector()[dofmap.cell_dofs(cell.index())], \
          dofmap.cell_dofs(cell.index()), \
          np.unique(chi0.vector().array())
    chi0.vector()[dofmap.cell_dofs(cell.index())] = 1
  else:
    print chi1.vector()[dofmap.cell_dofs(cell.index())], \
          dofmap.cell_dofs(cell.index()), \
          np.unique(chi1.vector().array())
    chi1.vector()[dofmap.cell_dofs(cell.index())] = 1

v1 = project(chi0*u, V)
v1 += project(chi1*mu*u, V)

print chi1.vector().array()
